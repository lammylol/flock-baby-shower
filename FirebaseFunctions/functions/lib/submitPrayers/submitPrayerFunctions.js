...
import { cleanFirestoreUpdate, updateAggregatedEmbeddingAndMetadataForTopic, enqueueRemoveArrayFieldUpdates, enqueueReplaceArrayFieldUpdates, convertToFirestoreTimestamp, } from '../utils/upload/firestoreUtils.js';
...
export const submitPrayerWithPoints = functions.https.onCall(async (request) => {
    ...
    const transaction = await db.runTransaction(async (tx) => {
        ...
        for (const { point, snap } of pointSnaps) {
            ...
            if (!fieldMap.prayerPoints)
                fieldMap.prayerPoints = [];
    ...
    try {
        const result = await db.runTransaction(async (tx) => {
            ...
            const addRefs = Array.isArray(addTopicIds) && addTopicIds.length > 0
                ? addTopicIds.map((id) => db.collection('prayerTopics').doc(id))
                : [];
            const removeRefs = Array.isArray(removeTopicIds) && removeTopicIds.length > 0
                ? removeTopicIds.map((id) => db.collection('prayerTopics').doc(id))
                : [];
            ...
            topicRefs.forEach((ref, i) => {
                topicMap.set(ref.id, topicSnaps[i]);
            });
            // === ADD ===
            // === Update topics ===
            if (addTopicIds.length > 0) {
                await Promise.all(addTopicIds.map(async (topicId) => {
        ...
        await db.runTransaction(async (transaction) => {
            ...
            await Promise.all(result.addedTopicIds.map((tid) => updateAggregatedEmbeddingAndMetadataForTopic(tid, transaction)));
        });
        return {
            success: true,
            ...
            journeyEntries: addTopicIds.map((topicId) => ({
                id: topicId,
