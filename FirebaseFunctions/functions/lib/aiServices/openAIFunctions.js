 ...
 */
export const analyzePrayerContent = functions.https.onCall({ secrets: [openAISecret] }, async (request) => {
    if (!request.auth) {
        throw new functions.https.HttpsError("unauthenticated", "The function must be called while authenticated.");
    }
    const { content, hasTranscription = false, maxPrayerPoints = 10, } = request.data;
    if (!content || typeof content !== "string" || !content.trim()) {
        throw new functions.https.HttpsError("invalid-argument", "No prayer content provided");
    }
    try {
        const openai = await initializeOpenAI();
        const titlePrompt = "Title: A concise title for the prayer, with a maximum character limit of 10.";
      ...
      ${rules}`;
        ...
        const processedTags = result.tags
            ...
            .slice(0, 2);
        const processedPrayerPoints = result.prayerPoints.map((prayerPoint) => {
        ...
        if (error.status === 429) {
            ...
            throw new functions.https.HttpsError("unauthenticated", "Authentication error with AI service. Please try again later.");
        }
        else {
            throw new functions.https.HttpsError("internal", error.message || "AI service error. Please try again later.");
        }
    }
});
export async function getVectorEmbeddingFromOpenAI(input) {
 ...
 */
export const getVectorEmbeddings = functions.https.onCall({ secrets: [openAISecret] }, async (request) => {
    if (!request.auth) {
