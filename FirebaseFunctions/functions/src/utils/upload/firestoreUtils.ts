...
import type { PrayerPoint, PrayerTopic } from '../../shared/types/firebaseTypes.js';

export function stripLocalFields<T extends Record<string, any>>(entity: T): T {
  const { isNew, ...rest } = entity;
  return rest as T;
}

// Add updatedAt: serverTimestamp() to a Firestore document in a transaction.
export function setFirestoreUpdatedAt(
...
export async function fetchTopicAndLinkedPoints(topicId: string) {
  ...
  if (!topicSnap.exists) return { topicData: null, prayerPoints: [] };
 ...
 */
export function extractAllEmbeddings(topicData: any, prayerPoints: any) {
  const allEmbeddings = [];
  ...
  if (prayerPoints.length > 0) {
    prayerPoints.forEach((point: any) => {
 ...
 */
export function extractDistinctPrayerTypes(prayerPoints: PrayerPoint[]) {
  const types = new Set();
 ...
 */
export async function aggregateTopicMetadata(topicId: string) {
 ...
 */
export async function updateAggregatedEmbeddingAndMetadataForTopic(
  topicId: string,
  transaction: FirebaseFirestore.Transaction
) {
...
export async function enqueueReplaceArrayFieldUpdates({
  transaction,
  snap,
  fieldMap,
}: {
  transaction: FirebaseFirestore.Transaction;
  snap: FirebaseFirestore.DocumentSnapshot;
  fieldMap: Record<string, Array<any>>; // Accept mixed value types
}) {
  ...
  for (const [field, newValues] of Object.entries(fieldMap)) {
    ...
    if (isObjectField) {
      ...
      const existingIds = new Set(
        existingArray
          .filter((item: any) => typeof item === 'object' && item !== null && 'id' in item)
          .map((item: any) => item.id)
      );

      // Add new objects that don't already exist
      const newObjects = safeValues.filter((v) =>
        typeof v === 'object' && v !== null && !existingIds.has(v.id)
      );
      ...
      const existingStrings = new Set(
        existingArray.filter((item: any) => typeof item === 'string')
      );
...
export async function enqueueRemoveArrayFieldUpdates({
  transaction,
  snap,
  fieldMap,
}: {
  ...
  for (const [field, removeValues] of Object.entries(fieldMap)) {
    ...
    const existingArray = Array.isArray(snap.get(field)) ? snap.get(field) : [];
    if (existingArray.length === 0) continue;

    const filtered = existingArray.filter((item: any) => {
...
export const removeEmptyArrays = (obj: any) => {
  return Object.fromEntries(
    Object.entries(obj)
      .map(([key, value]) => {
...
export const cleanFirestoreUpdate = (obj: any) => {
...
export function convertToFirestoreTimestamp(timestamp: any): Timestamp {
  if (!timestamp) {
