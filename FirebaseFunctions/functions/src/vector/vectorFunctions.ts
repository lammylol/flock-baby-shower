...
import type { LinkedPrayerEntity, PrayerPoint } from '../../shared/types/firebaseTypes.js';
...
export const findSimilarPrayersBatch = functions.https.onCall(async (request) => {
  ...
  try {
    ...
    const results = queryPrayerPoints.map((point: PrayerPoint, index: number) => {
...
export const updateAggregatedEmbeddingForTopicCallable = functions.https.onCall(async (request) => {
  ...
  if (!topicId) throw new functions.https.HttpsError('invalid-argument', 'Missing topicId');
...
export const onPrayerPointWrite = functions.https.onCall(async (request) => {
  ...
  if (!pointId) throw new functions.https.HttpsError('invalid-argument', 'Missing pointId');

  await db.runTransaction(async (transaction) => {
    const docRef = db.collection('prayerPoints').doc(pointId);
    const docSnap = await docRef.get();
    if (!docSnap.exists) throw new Error(`Prayer point ${pointId} not found`);

    const after = docSnap.data();

    const topicIds = new Set((after?.linkedTopics || []).map((t: { id: string }) => t.id));
    await Promise.all(Array.from(topicIds).map((topicId) => updateAggregatedEmbeddingAndMetadataForTopic(topicId as string, transaction)));
  });
  return { status: 'completed' };
});
