...
describe('OpenAI Cloud Functions', () => {
    ...
    describe('Error handling', () => {
        ...
        it('should reject when content is empty', async () => {
            await expect(
                analyzeFunction({ data: { content: '' }, auth: { uid: testUid, token: { email: 'test@example.com' } } } as any)
            ).rejects.toThrow();
        }, 10000);

        it('should reject when request is unauthenticated', async () => {
            await expect(
                analyzeFunction({ data: { content: 'Test prayer' } } as any)
            ).rejects.toThrow();
        }, 10000);

        it('should reject when input is empty', async () => {
            await expect(
                embeddingsFunction({ data: { input: '' }, auth: { uid: testUid, token: { email: 'test@example.com' } } } as any)
            ).rejects.toThrow();
        }, 10000);
    });
});